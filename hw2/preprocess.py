...
# x_mean = np.mean(x_analog.T, axis=0)
# print(x_mean)
# print(x_mean.shape)
x_norm_para = np.stack((np.mean(x_analog.T, axis=0), np.std(x_analog.T, axis=0)))
pd.DataFrame(x_norm_para).to_csv('para/norm_para', index=False)
x_std = np.concatenate((standardize(x_analog.T), x_digital.T), axis=1)
# x_std = x_data_and_test
#print(x_std.shape)
#print(x_std)
